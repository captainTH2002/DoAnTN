...
def row_bbs(bbs):
    if not bbs:
        return []  # Trả về danh sách rỗng nếu bbs rỗng
    ...
    sorted_clusters = [cluster for cluster, _ in zip_clusters]  # Chỉ lấy các cụm
    return sorted_clusters

def sort_json(json_data):
    """Sort the JSON data by position."""
    # Kiểm tra JSON có shapes hợp lệ
    if not json_data.get('shapes'):
        raise ValueError("JSON data has no shapes")
    
    bbs, labels, texts = [], [], []
    for shape in json_data['shapes']:
        if not shape.get('points') or len(shape['points']) != 4:
            continue  # Bỏ qua shape không hợp lệ
        ...
        bbs.append(bb)
        labels.append(shape.get('label', 'text'))
        texts.append(shape.get('text', ''))

    if not bbs:
        raise ValueError("No valid shapes found in JSON data")
...
def load_json_with_fallback(filepath, encodings=['utf-8', 'latin-1', 'cp1252']):
...
def get_input_from_json(json_fp, word_encoder, use_emb, emb_range):
    ...
    # Đọc JSON với load_json_with_fallback
    json_data = load_json_with_fallback(json_fp)
    
    # Kiểm tra JSON hợp lệ
    if not json_data.get('shapes'):
        raise ValueError(f"JSON file {json_fp} has no shapes")
    for shape in json_data['shapes']:
        if not shape.get('points') or len(shape['points']) != 4:
            raise ValueError(f"Invalid shape in {json_fp}: points must have exactly 4 coordinates")
    
    # Lấy đường dẫn ảnh
    img_fp = get_img_fp_from_json_fp(Path(json_fp))
    if img_fp is None:
    ...
    # Tạo tensor edges với dtype=torch.int64
    edges = torch.tensor(edges, dtype=torch.int64) if edges else torch.zeros((0, 3), dtype=torch.int64)
    edges = torch.unique(edges, dim=0)
    
    # Tạo edge_index và edge_type
    edge_index = edges[:, :-1].t().contiguous() if edges.size(0) > 0 else torch.zeros((2, 0), dtype=torch.int64)
    edge_type = (edges[:, -1] - 1) if edges.size(0) > 0 else torch.zeros((0,), dtype=torch.int64)
    
    # Kiểm tra và in cảnh báo nếu không có cạnh
    if edge_index.size(1) == 0:
        print(f"Warning: No edges generated for {json_fp}")
    ...
    return (
        ...
        edge_type.to(torch.int64),
    )
def inference_single_image(json_path, ckpt_path, output_dir, mart_name="congvan"):
    """Suy luận trên một ảnh và file JSON đơn lẻ hoặc tất cả ảnh trong một thư mục."""
    ...
    # Kiểm tra xem json_path là file hay thư mục
    path = Path(json_path)
    json_files = []
    
    if path.is_file():
        # Nếu là file, thêm vào danh sách xử lý
        json_files.append(path)
    elif path.is_dir():
        # Nếu là thư mục, lấy tất cả file JSON trong thư mục
        json_files = list(path.glob("*.json"))
        print(f"Tìm thấy {len(json_files)} file JSON trong thư mục {path}")
    else:
        raise FileNotFoundError(f"Không tìm thấy file hoặc thư mục: {json_path}")
    ...
    for json_fp in json_files:
        try:
            ...
            if img_fp:
                dest_img_path = os.path.join(output_dir, img_fp.name)
                # Chỉ sao chép nếu không phải cùng một file
                if str(img_fp) != dest_img_path:
                    from shutil import copyfile
                    copyfile(img_fp, dest_img_path)

            print(f"Đã xử lý xong {json_fp.name}. Kết quả lưu tại {output_dir}")
            
        except Exception as e:
            print(f"Lỗi khi xử lý file {json_fp.name}: {str(e)}")
            continue

def main(image_input):
    base_dir = os.path.dirname(os.path.abspath(__file__))
    ckpt_path = os.path.join(base_dir, "models/exp18_rgcn_7_layers_augment/model-epoch=09-train_loss=0.217-val_loss=0.189-val_f1=0.986.ckpt")
    mart_name = "congvan"
    ...
    if input_path.is_file() and input_path.suffix.lower() in ('.png', '.jpg', '.jpeg'):
        ...
        if not json_fp.exists():
            print(f"JSON file not found for {input_path}")
            return
        output_dir = input_path.parent
        inference_single_image(str(json_fp), ckpt_path, str(output_dir), mart_name)
    
    elif input_path.is_dir():
        # Xử lý tất cả ảnh trong thư mục
        for filename in os.listdir(input_path):
            if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                json_fp = input_path / filename.replace(filename.split('.')[-1], 'json')
                if not json_fp.exists():
